...
struct regulator_desc {
	...
	int id;
	unsigned n_voltages;
	const struct mt_regulator_ops *ops;
...
#define MT6359_BUCK(_match, _name, min, max, step,		\
...
#define MT6359_LDO_LINEAR(_match, _name, min, max, step,		\
...
#define MT6359_LDO(_match, _name, _volt_table,			\
...
#define MT6359_REG_FIXED(_match, _name, _enable_reg,	\
...
#define MT6359P_LDO1(_match, _name, _ops, _volt_table,	\
	_enable_reg, _enable_mask, _status_reg,		\
	_vsel_reg, _vsel_mask)				\
[MT6359_ID_##_name] = {					\
	.match = _match,					\
...
static int mt6359_regulator_get_mode(struct mt6359_plat *plat)
{
	struct mt6359_regulator_info *info = plat->info;
	int ret, regval;

	ret = regmap_read(plat->regmap, info->modeset_reg, &regval);
	if (ret != 0) {
		dev_err(plat->dev,
	...
	regval >>= __ffs(info->modeset_mask);

	if (regval == MT6359_BUCK_MODE_FORCE_PWM)
		return REGULATOR_MODE_FAST;

	ret = regmap_read(plat->regmap, info->lp_mode_reg, &regval);
	if (ret != 0) {
		dev_err(plat->dev,
...
static int mt6359_regulator_set_mode(struct mt6359_plat *plat, int mode)
{
	struct mt6359_regulator_info *info = plat->info;
	int ret = 0, val;
	int curr_mode;

	curr_mode = mt6359_regulator_get_mode(plat);
	switch (mode) {
	case REGULATOR_MODE_FAST:
		val = MT6359_BUCK_MODE_FORCE_PWM;
		val <<= __ffs(info->modeset_mask);
		ret = regmap_update_bits(plat->regmap,
					 info->modeset_reg,
					 info->modeset_mask,
					 val);
		break;
...
static int mt6359p_vemc_set_voltage_sel(struct mt6359_plat *plat, u32 sel)
{
	struct mt6359_regulator_info *info = plat->info;
	int ret;
	u32 val = 0;

	sel <<= __ffs(info->desc.vsel_mask);
	ret = regmap_write(plat->regmap, MT6359P_TMA_KEY_ADDR, TMA_KEY);
	if (ret)
		return ret;

	ret = regmap_read(plat->regmap, MT6359P_VM_MODE_ADDR, &val);
	if (ret)
		return ret;
	...
	ret = regmap_write(plat->regmap, MT6359P_TMA_KEY_ADDR, 0);
	return ret;
}

static int mt6359p_vemc_get_voltage_sel(struct mt6359_plat *plat)
{
	struct mt6359_regulator_info *info = plat->info;
	int ret;
	u32 val = 0;
	...
	val >>= __ffs(info->desc.vsel_mask);

	return val;
}

static int linear_get_value(struct mt6359_plat *plat)
{
	struct regulator_desc *desc = &plat->info->desc;
	int ret;
	int uV;
	uint val;
	uint i;
	...
	dev_dbg(plat->dev, "%s:\n", __func__);
	ret = regmap_read(plat->regmap, desc->vsel_reg, &val);
	if (!ret) {
		i = val & desc->vsel_mask;
		i >>= __ffs(desc->vsel_mask);
		if (i >= desc->n_voltages)
			return -EINVAL;
		uV = desc->min_uV + (i * desc->uV_step);
		dev_dbg(plat->dev, "%s: %d of %d %d, 0x%x: 0x%x 0x%x\n", __func__,
			i, desc->n_voltages, uV,
			desc->vsel_reg, val, desc->vsel_mask);
		return uV;
	}
	return ret;
}

static int linear_set_value(struct mt6359_plat *plat, int uV)
{
	struct regulator_desc *desc = &plat->info->desc;
	uint val;
	int i;
	...
	val = uV - desc->min_uV;
	i = (val + desc->uV_step - 1) / desc->uV_step;
	if (i >= desc->n_voltages)
		return -EINVAL;
	val = i << __ffs(desc->vsel_mask);
	dev_dbg(plat->dev, "%s: %d of %d, step %d, 0x%x\n", __func__, i, desc->n_voltages, desc->uV_step, val);
	return regmap_update_bits(plat->regmap, desc->vsel_reg, desc->vsel_mask, val);
}

static int regmap_get_enable(struct mt6359_plat *plat)
{
	struct regulator_desc *desc = &plat->info->desc;
	unsigned int enable_mask = desc->enable_mask;
	int ret;
	uint val;

	ret = regmap_read(plat->regmap, desc->enable_reg, &val);
	if (!ret)
		ret = (val & enable_mask) ? 1 : 0;
	return ret;
}

static int regmap_set_enable(struct mt6359_plat *plat, bool enable)
{
	struct regulator_desc *desc = &plat->info->desc;
	unsigned int enable_mask = desc->enable_mask;

	return regmap_update_bits(plat->regmap, desc->enable_reg,
		enable_mask, enable ? enable_mask : 0);
}

struct mt_regulator_ops {
	int (*get_value)(struct mt6359_plat *plat);
	int (*set_value)(struct mt6359_plat *plat, int uV);
	int (*get_enable)(struct mt6359_plat *plat);
	int (*set_enable)(struct mt6359_plat *plat, bool enable);
        int (*get_mode)(struct mt6359_plat *plat);
        int (*set_mode)(struct mt6359_plat *plat, int mode_id);
};

static const struct mt_regulator_ops mt6359_volt_linear_ops = {
	.get_value	= linear_get_value,
	.set_value	= linear_set_value,
	.get_enable	= regmap_get_enable,
	.set_enable	= regmap_set_enable,
	.get_mode	= mt6359_regulator_get_mode,
	.set_mode	= mt6359_regulator_set_mode,
};

static int table_get_value(struct mt6359_plat *plat)
{
	struct regulator_desc *desc = &plat->info->desc;
	int ret;
	int uV;
	...
	if (i >= desc->n_voltages)
		return -EINVAL;
	uV = desc->volt_table[i];
	dev_dbg(plat->dev, "%s: %d %d 0x%x: 0x%x 0x%x\n", __func__,
			i, uV,
			desc->vsel_reg, val, desc->vsel_mask);
	return uV;
}

static int table_set_value(struct mt6359_plat *plat, int uV)
{
	struct regulator_desc *desc = &plat->info->desc;
	int i;
	uint val;
...
static const struct mt_regulator_ops mt6359_volt_table_ops = {
	.get_value	= table_get_value,
	.set_value	= table_set_value,
	.get_enable	= regmap_get_enable,
	.set_enable	= regmap_set_enable,
};

static const struct mt_regulator_ops mt6359_volt_fixed_ops = {
	.get_enable	= regmap_get_enable,
	.set_enable	= regmap_set_enable,
};

static int vemc_table_get_value(struct mt6359_plat *plat)
{
	struct regulator_desc *desc = &plat->info->desc;
	int ret;
	int uV;
	...
	ret = mt6359p_vemc_get_voltage_sel(plat);
	if (ret < 0)
		return ret;
	i = ret;
	if (i >= desc->n_voltages)
		return -EINVAL;
	uV = desc->volt_table[i];
	dev_dbg(plat->dev, "%s: %d %d\n", __func__, i, uV);
	return uV;
}

static int vemc_table_set_value(struct mt6359_plat *plat, int uV)
{
	struct regulator_desc *desc = &plat->info->desc;
	int i;
	...
	dev_dbg(plat->dev, "%s: %d %d\n", __func__, i, desc->volt_table[i]);
	return mt6359p_vemc_set_voltage_sel(plat, i);
}

static const struct mt_regulator_ops mt6359p_vemc_ops = {
	.get_value	= vemc_table_get_value,
	.set_value	= vemc_table_set_value,
	.get_enable	= regmap_get_enable,
	.set_enable	= regmap_set_enable,
};
...
{
	struct pmic_chip *pmic = dev_get_priv(dev->parent);
	struct mt6359_regulator_info *info;
	ofnode rn;
	int i, hw_ver, cnt;
	int ret;
	...
	dev_dbg(dev, "%s:\n", __func__);
	rn = dev_read_subnode(dev, "regulators");
	if (!ofnode_valid(rn)) {
		dev_dbg(dev, "%s: regulators subnode not found!\n", __func__);
		return -ENXIO;
	}

	regmap_read(pmic->regmap, MT6359P_HWCID, &hw_ver);
	if (hw_ver >= MT6359P_CHIP_VER) {
		info = mt6359p_regulators;
		cnt = ARRAY_SIZE(mt6359p_regulators);
	} else {
		info = mt6359_regulators;
		cnt = ARRAY_SIZE(mt6359_regulators);
	}

	for (i = 0; i < cnt; i++, info++) {
		struct mt6359_plat *plat;
		struct udevice *new_dev;
		ofnode sn;

		sn = ofnode_find_subnode(rn, info->match);
		if (ofnode_valid(sn)) {
			ret = device_bind_driver_to_node(dev, "mt6359-regulators", info->match, sn, &new_dev);
			if (!ret) {
				plat = dev_get_plat(new_dev);
				plat->dev = new_dev;
				plat->regmap = pmic->regmap;
				plat->info = info;
				plat->index = i;
			}
		}
...
{
	struct mt6359_plat *plat = dev_get_plat(dev);
	const struct mt_regulator_ops *ops = plat->info->desc.ops;

	dev_dbg(dev, "%s:\n", __func__);
	if (ops && ops->get_value)
		return ops->get_value(plat);
	return 0;
}

static int mt6359_set_value(struct udevice *dev, int uV)
{
	struct mt6359_plat *plat = dev_get_plat(dev);
	const struct mt_regulator_ops *ops = plat->info->desc.ops;
...
static int mt6359_set_enable(struct udevice *dev, bool enable)
{
	...
	if (ops && ops->set_enable)
		return ops->set_enable(plat, enable);
	return 0;
}

static int mt6359_get_mode(struct udevice *dev)
{
	struct mt6359_plat *plat = dev_get_plat(dev);
	const struct mt_regulator_ops *ops = plat->info->desc.ops;
	...
	dev_dbg(dev, "%s:\n", __func__);
	if (ops && ops->get_mode)
		return ops->get_mode(plat);
	return 0;
}

static int mt6359_set_mode(struct udevice *dev, int mode_id)
{
	struct mt6359_plat *plat = dev_get_plat(dev);
	const struct mt_regulator_ops *ops = plat->info->desc.ops;
	...
	dev_dbg(dev, "%s: %d\n", __func__, mode_id);
	if (ops && ops->set_mode)
		return ops->set_mode(plat, mode_id);
	return 0;
}

static const struct dm_regulator_ops mt6359_ops = {
...
static int mt6359_of_to_plat(struct udevice *dev)
{
	struct mt6359_plat *plat = dev_get_plat(dev);

	dev_dbg(dev, "%s:\n", __func__);
	plat->enable_ramp_delay = dev_read_u32_default(dev, "regulator-enable-ramp-delay", 0);
	return 0;
}

U_BOOT_DRIVER(mt6359_regulators) = {
	.name = "mt6359-regulators",
	.id = UCLASS_REGULATOR,
	.ops = &mt6359_ops,
	.of_to_plat = mt6359_of_to_plat,
	.plat_auto = sizeof(struct mt6359_plat),
};

static const struct udevice_id mt6359_of_match[] = {
	{"mt6359-regulator", 0},
	{ /* sentinel */ },
};
