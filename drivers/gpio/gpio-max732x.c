...
{
	struct max732x_chip *chip = dev_get_plat(dev);
...
{
	return max732x_chip_get_value(dev_get_plat(dev), offset);
}

static int max732x_set_value(struct udevice *dev, unsigned offset, int val)
{
	max732x_chip_set_value(dev_get_plat(dev), offset, val);
	return 0;
}
...
U_BOOT_DRIVER(max732x) = {
	...
	.ops		= &max732x_ops,
	.probe		= max732x_probe,
	.plat_auto	= sizeof(struct max732x_chip),
