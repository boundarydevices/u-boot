...
#ifdef CONFIG_IMX8M
#include "hdp/API_AFE_t28hpc_hdmitx.h"
#endif
				  ...
				  struct video_mode_settings *vms)
{
	...
	pr_debug("Cadence VIC %3d, CEA VIC %3d: %4d x %4d %c @ %3d.%03d [%6d kHz] Vpol=%d Hpol=%d\n",
	      vic, cea_vic, vms->xres, vms->yres, iflag, frame_rate_Hz,
	      frame_rate_frac_Hz, pixel_clock_kHz, vms->vpol, vms->hpol);

	pr_debug(" mode timing fp sync bp h:%3d %3d %3d v:%3d %3d %3d\n",
			  ...
			  bool pixel_clk_from_phy)
{
	...
	ret = cdn_api_general_test_echo_ext_blocking(echo_msg,
						     ...
						     sizeof(echo_msg),
						     CDN_BUS_TYPE_APB);
	pr_debug("_General_Test_Echo_Ext_blocking - (ret = %d echo_resp = %s)\n",
	      ret, echo_resp);

	/* Configure PHY */
	character_freq_khz = phy_cfg_t28hpc(4, vic_mode, bps,
					    format, pixel_clk_from_phy);
	pr_debug("phy_cfg_t28hpc (character_freq_mhz = %d)\n",
	      character_freq_khz);
	...
	ret = cdn_api_general_write_register_blocking
		(ADDR_SOURCD_PHY + (LANES_CONFIG << 2),
		 ...
		 F_SOURCE_PHY_COMB_BYPASS(0) | F_SOURCE_PHY_20_10(1));
#endif
	pr_debug("_General_Write_Register_blocking LANES_CONFIG ret = %d\n", ret);

	ret = CDN_API_HDMITX_Init_blocking();
	pr_debug("CDN_API_STATUS CDN_API_HDMITX_Init_blocking  ret = %d\n", ret);

	ret = CDN_API_HDMITX_Init_blocking();
	pr_debug("CDN_API_STATUS CDN_API_HDMITX_Init_blocking  ret = %d\n", ret);

	ret = CDN_API_HDMITX_Set_Mode_blocking(ptype, character_freq_khz);
	pr_debug("CDN_API_HDMITX_Set_Mode_blocking ret = %d\n", ret);

	ret = cdn_api_set_avi(vic_mode, format, bw_type);
	pr_debug("cdn_api_set_avi  ret = %d\n", ret);

	ret =  CDN_API_HDMITX_SetVic_blocking(vic_mode, bps, format);
	pr_debug("CDN_API_HDMITX_SetVic_blocking ret = %d\n", ret);
