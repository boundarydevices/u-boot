...
static int of_parse_phandle1(ofnode np, const char *propname, ofnode *ph)
{
...
static int lt8912_ofdata_to_platdata(struct udevice *dev)
{
	...
	ret = of_parse_phandle1(np, "display-dsi", &lt->disp_dsi);
	if (ret) {
		debug("display-dsi %d\n", ret);
		return ret;
	}
	...
	ret = of_parse_phandle1(np, "ddc-i2c-bus", &ddc);
	if (!ret) {
...
U_BOOT_DRIVER(lt8912) = {
	...
	.of_to_plat = lt8912_ofdata_to_platdata,
	.probe	= lt8912_probe,
	.ops	= &lt8912_ops,
	.priv_auto = sizeof(struct lt),
};
