...
#define ALL_KSZ_PHY_MASK(a) (KSZ9021_MASK(a))
...
static void phy_ar8031_config(struct phy_device *phydev)
{
	...
	freq = getenv_ulong("phy_clock_out", 10, 125000000);
	if (freq >= 125000000) {
		regval |= 0x18;
...
static void phy_micrel_config(struct phy_device *phydev)
{
	if (((phydev->drv->uid ^ PHY_ID_KSZ9031) & 0xfffffff0) == 0) {
		u32 freq = getenv_ulong("phy_clock_out", 10, KSZ_CLK_DEFAULT);
		u32 led_mod = getenv_ulong("phy_led_mode", 10, 1);
