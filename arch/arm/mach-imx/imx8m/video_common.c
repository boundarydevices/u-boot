...
do { \
	...
	if ((temp & (mask)) == ((val) & (mask))) \
		...
		pr_debug("%s():%d 0x%08x -> 0x%08x instead of 0x%08x\n", \
		      ...
		      int p_clk, int rtr_clk)
{
	...
	if (rtr_clk == 400) {
		...
		pr_debug("rtr_clk does not match a supported frequency");
	}
	...
	if (hdmi_core_clk == 200) {
		...
		pr_debug("hdmi_core_clk does not match a supported frequency");
	}
#endif
	...
	if (p_clk == 27) {
		...
		pr_debug("p_clk does not match a supported frequency");
	}
#endif
}
...
static int imx8m_power_init(uint32_t clock_control)
{
	...
	pr_debug("%s(): DCSS clock control 0x%08x\n", __func__, temp);
...
void *video_hw_init(void)
{
	...
	pr_debug("IMXDCSS display started ...\n");
		    ...
		    uint8_t disp, uint32_t pixfmt)
{
	...
	if (disp > 1) {
		pr_debug("Invalid disp parameter %d for imxdcss_fb_init()\n",
		      disp);
		return -EINVAL;
	}
