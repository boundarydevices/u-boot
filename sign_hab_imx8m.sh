...
for i in flash.bin binman.log .config; do
	if [ ! -f $i ]; then
...
# update SPL values
SPL_START_ADDR=`awk '/spl hab block/{print $4}' binman.log`
SPL_OFFSET=`awk '/spl hab block/{print $5}' binman.log`
SPL_LENGTH=`awk '/spl hab block/{print $6}' binman.log`
...
IVTOFFSET="0x1000"
IVTSIZE="0x20"
FIT_OFFSET=`awk '/File.*image-pos/{print $9}' binman.log | tail -n 1`
...
CSF_SPL_OFFSET=`awk '/csf_off/{print $2}' binman.log | head -n 1`
